...
public class MermaidAbility extends AbilityNoInfo {
    ...
    public boolean canUse(AbstractSquareBoard board, Piece piece, Point start) {
        return commonCanUse(board, piece) && board.getMatchingEscaques(PATTERN, start).stream().anyMatch(e -> e.getPiece() instanceof Portal);
    }

    @Override
    public void use(AbstractSquareBoard board, Piece piece, Point start) {
        board.getMatchingEscaques(PATTERN, start).stream().filter(e -> e.getPiece() instanceof Portal).forEach(e -> e.getPiece().changeCD(-2));;
